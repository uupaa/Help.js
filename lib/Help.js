...
//}@assert
    ...
    // --- dump body and online help ---
    var searchURL    = _getSearchURL(fn, path);
    var referenceURL = _getReferenceURL(fn, _extractSharp(Help_getHelpAttribute(fn) || path));

    var body = _inBrowser ? _stylishBody(fn, highlight)
                          : [fn + ""];
    var help = _inBrowser ? _stylishHelp([searchURL, referenceURL])
                          : [searchURL, referenceURL].join("\n");

    console.log.apply(console, body);
    console.log.apply(console, help);
}
...
function _stylishHelp(urls) {
    var style = [];
    var help = "\u25b6 " + urls.join("\n\u25b6 ").replace(/%c/g, "% c").
                               replace(/(http[^\n]+)/g, function(_, url) {
                    style.push("border-bottom:2px solid #9ff", "");
                    return "%c" + url + "%c\n";
                });
...
function _getSearchURL(fn,     // @arg Object/Function: Object.freeze
                       path) { // @arg String: "Object.freeze"
                               // @ret URLString: Google.search url.
    //
...
function _getReferenceURL(fn,     // @arg Object/Function: Object.freeze
                          path) { // @arg String: "Object.freeze"
                                  // @ret URLString: JavaScript/WebModule reference url.
    var className  = path.split(".")[0] || "";       // "Array.prototype.forEach" -> ["Array", "prototype", "forEach"] -> "Array"
...
function Help_getFunctionPath(fn) { // @arg Function: object. eg: Object.freeze
    ...
    var path = "";
    var ignoreClass = ["webkitStorageInfo", "Infinity", "NaN"];
    var ignoreKeys  = ["arguments", "caller", "callee", "buffer", "byteOffset", "byteLength", "length"];
    ...
    globalClasses.some(function _findClass(className) {
        if (ignoreClass.indexOf(className) < 0) {
            if (global[className] === fn) {
                path = className;
            }
            if ( !path && /object|function/.test(typeof global[className]) ) {
            ...
            if ( !path && "prototype" in global[className] ) {
