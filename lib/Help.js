...
//}@assert
    ...
    if (_inBrowser && keyword) {
        _dumpFunctionBody(fn, keyword);
    } else {
        console.log( fn || "" );
    }
    ...
    function _dumpFunctionBody(fn, keyword) {
        var style = [];
        var body = (fn + "").replace(/%c/g, "% c").
                             replace(RegExp(keyword, "g"), function() {
                        style.push("background:#ff9", "");
                        return "%c" + keyword + "%c";
                    });
        console.log.apply(console, [body].concat(style));
    }

    function _dumpHelpURL(urls) {
        var style = [];
        ...
        console.log.apply(console, [result].concat(style));
    }
}
...
Help["url"]                  = Help_url;                  // Help.url(fn:Function):Object
...
// --- implement -------------------------------------------
function Help_url(fn) { // @arg Function:
                        // @ret Object: { reference, repository, search }
                        ...
                        // @help: Help.url
    return _collect( fn, Help_reflection(fn) );
}
                ...
                if (typeof global[className] === "function") {
                    ...
                    if ("prototype" in global[className]) {
                        return keys(global[className]["prototype"]).some(function(method) {
                                    try {
                                        // Function.prototype.help -> skip
                                        if (className === "Function" && method === "help") {
                                            return false;
                                        }
